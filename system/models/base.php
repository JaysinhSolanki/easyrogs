<?php
  ...
  abstract class BaseModel
  {
    ...
    // search $term in $fields of $tableName with $conditions
    public function searchBy( $tableName, $term, $fields, $conditions )
    {
      $whereConditions = ["1 = 1"];
      foreach( $conditions as $name => $value ) {
        if ( is_array($value) ) {
          $inQuery = '(' . implode(',', $value) . ')'; // TODO: wont work for strings
          $whereConditions[] = "$name IN $inQuery";
        }
        else {
          $whereConditions[] = "$name = :$name";
        }

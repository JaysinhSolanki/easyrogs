...
class Side extends BaseModel
{
  const CASE_DATA_FIELDS = [
    ...
    'discovery_cutoff', 'county_name', 'masterhead', 'letterhead', 'normalized_number', 'header_height', 'footer_height'
  ];
  ...
  function __construct($dbConfig = null)
  {
    ...
    $this->queryTemplates = array_merge($this->queryTemplates, [
      ...
      'getSideTeam' =>  'SELECT u.*, su.active AS side_active
FROM system_addressbook AS u
     INNER JOIN sides_users AS su
       ON su.system_addressbook_id = u.pkaddressbookid 
     INNER JOIN sides AS s
...
FROM sides_users AS su WHERE su.side_id = :side_id AND is_deleted = 0',
    ...
    'getCaseLetterHead' =>  'SELECT * FROM sides WHERE case_id = :case_id AND primary_attorney_id= :user_id AND is_deleted = 0'
  ...
  {
    $query = $this->queryTemplates['getUsersFlag'];
    return User::publishable(
      $this->readQuery($query, ['side_id' => $sideId])
    );
  }
  ...
  function updateSide($sideId, $fields)
  {
    ...
    // $test =  $this->update('sides', $fields, ['id' => $sideId]);
  ...
  {
    function checkMergeable($carry, $item)
    { // $carry = [mainSide, mergeable, sidesModel]
      $sides = $carry[2];
  ...
  function updateCaseData($sideId, $data)
  {
    ...
    //$caseData['letterhead'] = $data['letterhead'] ?? null;

    if(empty($data['letterhead'])){
      //$userData   = $usersModel->find($data['case_attorney']);
      //$caseData['letterhead'] = $userData['letterhead'];
      $caseData['letterhead'] = null;
    } else {
      $caseData['letterhead'] = $data['letterhead'];
    }

    if(empty($data['header_height'])){
      //$userData   = $usersModel->find($data['case_attorney']);
      //$caseData['header_height'] = $userData['header_height'];
      $caseData['header_height'] = null;
    } else {
      $caseData['header_height'] = $data['header_height'];
    }

    if(empty($data['footer_height'])){
      //$userData   = $usersModel->find($data['case_attorney']);
      //$caseData['footer_height'] = $userData['footer_height'];
      $caseData['footer_height'] = null;
    } else {
      $caseData['footer_height'] = $data['footer_height'];
    }
  ...
  function findAttorneyCaseLetterHead($caseId, $userId){
    $query = $this->queryTemplates['getCaseLetterHead'];

...
$discoverydetails	= $AdminDAO->getrows('discoveries',"*",
								"uid = :uid",
								array(":uid"=>$discovery_uid));
if( sizeof($discoverydetails) ) {
	...
	$discoveryresponses	= $AdminDAO->getrows('responses',"*",
												"fkdiscoveryid = :fkdiscoveryid",
												array("fkdiscoveryid"=>$discovery_id) );
	if( !empty($discoveryresponses) ) {
		foreach( $discoveryresponses as $response_data ) {
			$response_id = $response_data['id'];
			$AdminDAO->deleterows('responses',"id = :id",
										array("id"=>$response_id) );
			$AdminDAO->deleterows('response_questions'," fkresponse_id = :fkresponse_id",
										array("fkresponse_id"=>$response_id));
		}
	}

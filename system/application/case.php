...
if ($isDraft) {
	$casesModel->cleanupSides($caseId);
}
...
<script type="text/javascript">
	...
	$(document).on('change', '#case_attorney', () => {
		setMastHead()
	});


	function loadServiceListModal(caseId, userId = null, slAttorneyId = null) {
		$.post("loadservicelistmodal.php", {
			case_id: caseId,
			user_id: userId,
			sl_attorney_id: slAttorneyId
		}).done(function(resp) {
	...
	function calculated_discovery_cutoff_date(trail_date) {
		$.post("calculatecutoffdateaction.php", {
	...
	function loadmasterhead() {
		...
		$.post("loadmasterhead.php", {
			case_attorney_email: case_attorney_email,

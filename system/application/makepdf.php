...
if ($type == Discovery::TYPE_EXTERNAL) {
    ...
    $attorneyDetails    = $AdminDAO->getrows(
        'attorney a,client_attorney ca,system_addressbook sa',
        "*",
...
if (in_array($form_id, array(Discovery::FORM_CA_FROGS, Discovery::FORM_CA_FROGSE))) {
    ...
    if (sizeof($isConWithDiscovery)) {
        ...
        if ($is_served) {
            $con_Details = [
                "con_discovery_name" => "REQUESTS FOR ADMISSION",
...
if (in_array($form_id, [Discovery::FORM_CA_RFAS])) {
    ...
    if ($in_conjunction) {
        $con_Details = [
            "con_discovery_name" => $con_discovery,

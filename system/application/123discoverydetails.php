...
<div id="screenfrmdiv" style="display: block;">
    <div class="col-lg-12">
        <div class="hpanel">
            <div class="panel-heading"><h3><?php echo $form_name;?>
             </h3> </div>
                ...
                if(in_array($form_id,array(1,2)))
                {
                    foreach($mainQuestions as $data)
                    {
						...
						**/
                        ...
                        ?>
                        <li <?php if($depends_on_question != 0) {?>class="list-group-item row_<?php echo $depends_on_question; ?>" <?php if(($dependent_answer == 'No' || $dependent_answer == '') && $view != 1){ ?>style='display:none;' <?php } }else {?> class="list-group-item"  <?php } ?>>
                            <?php
                                    ...
                                    else 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            <p> 
                                                ...
                                                <textarea style="background-color: antiquewhite;"  id="objection_<?php echo $discovery_question_id; ?>" class="form-control" name="objection[<?php echo $discovery_question_id ?>]" placeholder="Your Objection" required><?php echo html_entity_decode($objection) ?></textarea>
                                                <?php
                                                }
                                                ?>
                                            </p>
                                                    ...
                                                    ?>
                                                     <li class="list-group-item">
                                                        <div class="form-group">
                                                            ...
                                                            <?php echo $css ?> 
                                                            <?php if(($answer == "No" || $answer == "") && $p_q_type_id != 3){ ?> disabled <?php } ?>><?php echo html_entity_decode($answer1) ?></textarea>
                                                        </div>  
                                                    </li>   
                                                    <?php
                                                    }
						...
						<li class="list-group-item">
							...
							<div class="form-group">
								...
								<div class="form-group">
									...
									<textarea  style="background-color: antiquewhite; <?php if($question_admit_id == 1){ ?> display:none <?php } ?> " 
									<?php if($answer == "Admit" || $answer == ""){ ?> disabled <?php } ?> 
                                    ...
                                    <textarea 
									<?php if($answer == "Admit" || $answer == ""){ ?> disabled <?php }  
									if($question_admit_id == 1){ ?> readonly <?php } ?> 
                                    ...
                                    placeholder="Answer" required <?php echo $css ?>><?php echo ($sub_answer_show); ?></textarea>
						...
						<li class="list-group-item">
							...
							<div class="form-group">
								...
								if($view != 1)
								{
									...
									{
									?>
										<textarea id="answer<?php echo $discovery_question_id ?>" class="form-control " name="answer[<?php echo $discovery_question_id; ?>]" placeholder="Answer" required <?php echo $css ?>><?php echo html_entity_decode($answer) ?></textarea>
									<?php
									}
									if($form_id == 5)
									{
									?>
										...
										<ul class="list-group" id="subdiv<?php echo $discovery_question_id;?>" <?php if($answer == 'Select Your Response' || $answer == "I have responsive documents"){ ?>style="display:none" <?php } ?>>
											...
											<div class="form-group">
												...
												<textarea <?php if($answer == 'Select Your Response' || $answer == "I have responsive documents"){ ?> disabled <?php } ?> id="subanswer<?php echo $discovery_question_id ?>" class="form-control" name="subanswer[<?php echo $discovery_question_id; ?>]" placeholder="Your Answer" required <?php echo $css ?>><?php echo html_entity_decode($answer_detail) ?></textarea>
											</div>
											</li>
										</ul> 
									<?php
            ...
            <ul class="list-group"> 
                <li class="list-group-item">
                    <div class="">
                        <p> 
                            <b>Documents related to this dicsovery.</b>
                        </p>
                    </div>
            ...
            {
            ?>
            <a href="javascript:;" class="btn btn-primary" onclick="serveFunction2('<?php echo $discovery_verification ?>','<?php echo $_GET['id']?>','<?php echo $response_id ?>')"  ><i class="fa fa-share"></i> Serve</a>
            <button type="button" class="btn btn-success buttonid" data-style="zoom-in" onclick="sendtoclientfunction('<?php echo $discovery_id ?>',2);" >
                    <i class="icon-ok bigger-110"></i>
...
function sendtoclientfunction(discovery_id,actiontype)
{
	...
	setTimeout(function()  
	{
		...
		{		 
			$.post( "messagetoclientforverification.php", { discover_id: discovery_id,actiontype:actiontype}).done(function( data ) 
			{
				$.LoadingOverlay("hide");
				response(data);
			});
...
function FunctionFinalDraftAction()
{
	$.post( "finaldraftaction.php",  $("#finaldraft").serialize()).done(function( data ) 
	{
    	if(data == "success")
		{
			...
			PopupForPOS('<?php echo $discovery_id?>','<?php echo $response_id?>');
		}
  	});
}

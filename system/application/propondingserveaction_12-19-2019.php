...
if($discovery_type == 1) //If served discovery is external then we save POS details with discovery because in external case we serve discovery.
{
	...
	$fields				=	array('pos_state','pos_city','pos_text','pos_updated_at','pos_updated_by','is_served','served');
...
//if($isemail==1)
{
	...
	///echo "<br>";
	$path			=	$_SESSION['system_path']."uploads/documents/".$filename;
	$docsArray[]	=	array("path" => $path, "filename" => $filename);
	...
	foreach($servicelists as $list)
	{
		...
		$attorney_id	=	$list['id'];
		/**
		* If attorney is not the member of Easy Rogs then send invite to him
		**/
		...
		If you're not already using EasyRogs, please join at <?php echo $emailURL ?>. Membership is free and the Service Charge is waived through December 31, 2019. EasyRogs makes discovery easy, saves time & money, and protects our planet.
        <br />
        --
